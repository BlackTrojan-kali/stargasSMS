...
class CommercialController extends Controller
{
    ...
    public function ventes(Request $request,$type){
        ...
        $accessories = Article::where("type","=","accessoire")->get("title");
        if($type =="versements"){
            $versements1 = Versement::where("bank","AFB")->where("region","=",Auth::user()->region)->where("service",Auth::user()->role)->get();
            $versements2 = Versement::where("bank","CCA")->where("region","=",Auth::user()->region)->where("service",Auth::user()->role)->get();
            return view("commercial.historique-versements",["stocks"=>$stocks,"accessories"=>$accessories,"ventes"=>$versements1,"ventes2"=>$versements2,"type"=>$type]);
        }else{
            $ventes = Vente::where("type",$type)->where("region","=",Auth::user()->region)->where("service",Auth::user()->role)->get();
