...
<h1 class="p-2 mt-5 font-bold text-2xl">Historique des {{ strtoupper($type) }}</h1>
    ...
    <table class="history scroll mt-10 w-full border-2 border-collapse border-gray-400 ">
        ...
        <tbody>
            ...
            <tr id="{{ $vente->id }}" class="hover:text-white hover:bg-blue-400 hover:cursor-pointer">
                ...
                <td>{{ $vente->address }}</td>
                <td>{{ ($vente->prix_6 * $vente->qty_6) + ($vente->prix_12 * $vente->qty_12) + ($vente->prix_50 * $vente->qty_50) }}</td>
                <td>{{ $vente->created_at }}</td>
                <td><a href="{{ route("printInvoice",["id"=>$vente->id]) }}"><i class="text-teal-900">print</i></a> <i class="text-red-500 delete">delete</i></td>
            </tr>
            @endforeach
        </tbody>
    </table>
