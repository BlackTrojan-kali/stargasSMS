...
.table td{
 ...
 <header >
        <div class="logo-section">
            <img src="{{'data:image/png;base64,'.base64_encode(file_get_contents(public_path('images/logo.png')))}}" width="150px">  <p>
...
<table>
    ...
    <tbody>
        ...
        <tr>
             ...
             <td>01 </td>
            <td> 12.5 {{ $type == "vente" ? "KG - GPL 12.5 KG LPG":"kG CONSIGNE" }}</td>
            <td>{{ $vente->qty_12 }}</td>
            <td>{{ $vente->prix_12 }} </td>
            <td>{{number_format(($vente->qty_12 * $vente->prix_12),2,","," ") }}</td>
        ...
        <tr>
            ...
            <td>02 </td>
           <td>6 {{ $type == "vente" ? "KG - GPL 6 KG LPG":"kG CONSIGNE" }}</td>
           <td>{{ $vente->qty_6 }}</td>
           <td>{{ $vente->prix_6 }} </td>
       ...
       <tr>
        ...
        <td>03 </td>
       <td>50 {{ $type == "vente" ? "KG - GPL 50 KG LPG":"kG CONSIGNE" }} </td>
       <td>{{ $vente->qty_50 }}</td>
       <td>{{ $vente->prix_50}} </td>
       <td>{{  number_format(($vente->qty_50 * $vente->prix_50),2,","," ") }}</td>
...
<div class="my-total">
    <p>Montant Total HT: {{ number_format(floatval($vente->prix_12 * $vente->qty_12) + floatval($vente->prix_6 * $vente->qty_6) + floatval($vente->prix_50 * $vente->qty_50),2,","," ")}}</p>

<p>Montant Total TTC : {{ number_format( floatval($vente->prix_12 * $vente->qty_12) + floatval($vente->prix_6 * $vente->qty_6) + floatval($vente->prix_50 * $vente->qty_50),2,","," ")}}</p>
</div>
</div>
<br>
<br>
...
<div class="comptes">
    ...
    <p class="contri"><i><b>Siège: Zone Industrielle MAGZI Adresse:6792 Yaoundé Tél:+237 691 50 03 05/ 691 50 03 08 Mail: <br>
    sales@stargascameroun.com/info@stargascameroun.com Num contribuable: M051200042049E Reg Commerce: YAO/2012/5/323 <br>
